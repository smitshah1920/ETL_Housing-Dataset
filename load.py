...
def insert_transactions():
    ...
    """
    # Retrieving all the transaction ids from the clean table
    ...
    # Printing total number of transactions to insert
    print("Transactions to insert:", transactions_to_insert.count())

    # Inserting the rows from the previously selected transactions
    stm = insert(PprCleanAll).from_select(
        ["date_of_sale", "address", "postal_code", "county", "price", "description"],
        transactions_to_insert,
    )

    # Executing and commit the statement to make changes in the database.
    session.execute(stm)
    session.commit()
...
def delete_transactions():
    ...
    """
    # Getting all ppr_raw_all transaction ids
    raw_transaction_ids = session.query(PprRawAll.transaction_id)

    # Filtering all the ppt_clean_all table transactions that are not present in the ppr_raw_all table
    # and deleting them.
