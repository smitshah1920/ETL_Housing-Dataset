...
# Sourcing the path where we want to save the .zip file downloaded from the website
...
def create_folder_if_not_exists(path):
    """
    Creating a new folder if it doesn't exists
    """
...
def download_snapshot():
    """
    Downloading a new snapshot from the source
    """
...
def save_new_raw_data():
    """-
    Saving new raw data from the current snapshot from the source
    """
    ...
    with tempfile.TemporaryDirectory() as dirpath:
        with ZipFile(
            ...
            with open(csv_file_path, mode="r", encoding="windows-1252") as csv_file:
                ...
                row = next(reader)  # Getting first row from reader
                ...
                with open(
                    ...
                    # Renaming field names so they're ready for the next step
                    fieldnames = {
                    ...
                    # Writing headers as first line
                    writer.writerow(fieldnames)
                    for row in reader:
                        # Writing all rows in file
                        writer.writerow(row)
