...
def update_description(description_input):
    """
    Simplifying the description field for potentialy future analysis, just return:
...
def update_price(price_input):
    """
    Returning price as integer by removing:
    - "€" symbol
...
def truncate_table():
    """
    Ensuring that "ppr_raw_all" table is always in empty state before running any transformations.
...
def transform_new_data():
    """
    Applying all transformations for each row in the .csv file before saving it into database
    """
    with open(raw_path, mode="r", encoding="windows-1252") as csv_file:
        # Read the new CSV snapshot ready to be processed
        reader = csv.DictReader(csv_file)
        # Initializing an empty list for our PprRawAll objects
        ppr_raw_objects = []
        for row in reader:
            # Applying  transformations and save as PprRawAll object
        ...
        # Saving all new processed objects and commit
